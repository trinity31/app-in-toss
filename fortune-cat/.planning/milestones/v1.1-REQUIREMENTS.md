# Requirements Archive: v1.1 복냥타로 통합 (별도 탭)
...
- [x] **NAV-01**: 사용자가 앱 하단 탭바에서 "사주"와 "타로"를 한 번의 탭으로 전환할 수 있다
- [x] **NAV-02**: 탭바에서 현재 활성 탭이 시각적으로 명확히 강조된다 (아이콘·컬러)
- [x] **NAV-03**: 기존 사주 흐름(`/`, `/saju`, `/new-year`, `/amulet`)이 탭바 도입 후에도 동일하게 동작한다

### Tarot Core (데일리 원카드)

- [x] **TAROT-01**: 사용자가 타로 탭에서 카드 뒷면을 보고 한 장을 뒤집어 오늘의 카드를 뽑을 수 있다 *(Phase 03 VERIFICATION.md 미생성 — implicit pass via Phase 04/05 검증 + UAT § 5 선택 항목 일부 통과)*
- [x] **TAROT-02**: 결과 화면에 카드 이미지·이름·해석 텍스트가 함께 표시된다 *(Phase 03 implicit pass; Quick task 260504-gbj 카드 확대 모달 추가)*
- [x] **TAROT-03**: 사용자가 결과 화면에서 "처음으로(intro 복귀)" 액션을 시작할 수 있다 *(v1.1 수익 모델 전환으로 '다시 뽑기' → '처음으로' 의미 변경)*

### Daily Lock (하루 한 번 + 자정 리셋)

- [x] **DAILY-01**: 사용자가 카드를 뽑은 후 같은 날(KST 자정 전) 타로 탭에 다시 진입하면, 새 카드를 뽑는 흐름 대신 그날 뽑은 카드의 결과 화면이 표시된다
- [x] **DAILY-02**: KST 자정(00:00) 이 지나면 lock 이 해제되어 사용자가 새 카드를 뽑을 수 있다
- [x] **DAILY-03**: 사용자가 앱을 재시작하거나 페이지를 새로고침해도 그날 뽑은 카드가 유지된다 (Toss Storage / localStorage 영속성)

### Sharing

- [x] **SHARE-01**: 사용자가 데일리 원카드 결과(카드/해석)를 토스 공유 시트로 외부에 공유할 수 있다

### Analytics

- [x] **ANL-01**: 사용자가 타로 탭에 진입하면 Firebase Analytics에 진입 이벤트가 기록된다 (`already_drawn` 플래그 포함) *(REVIEW WR-01 fix — useRef 가드로 중복 발화 차단)*
- [x] **ANL-02**: 사용자가 카드를 뽑으면 카드 뽑기 이벤트가 기록된다 (카드 식별자 포함)
- [x] **ANL-03**: 사용자가 결과를 공유하면 공유 이벤트가 기록된다
