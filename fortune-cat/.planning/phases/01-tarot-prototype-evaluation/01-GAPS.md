...
| **AIT 빌드 설정**    | `granite.config.ts` — `appName: "boknyang-tarot"`, `permissions: []`, `host: localhost`, dev 명령 `vite dev`                                                                                                                                                                                                                       | `granite.config.ts` — `appName: 'fortune-cat'`, `permissions: [camera, photos]`, `host: 192.168.35.153`, `bridgeColorMode: basic`, `navigationBar: { withBackButton, withHomeButton }`, dev 명령 `vite --host` | 🟢        | fortune-cat의 `granite.config.ts`를 **변경하지 않음**(타로는 별도 라우트 추가 없이 탭바로 통합되므로 권한·설정 변경 불필요). PROJECT.md 제약: `granite.config.ts` 권한 변경 시 토스 심사 영향 검토 — 본 마일스톤에서는 변경 없음. boknyang-tarot의 `permissions: []`은 fortune-cat의 기존 카메라/앨범 권한과 무관.                                                                            | (해당 없음 — 영향 미미) |
...
_Phase: 01-tarot-prototype-evaluation_
_Plan: 01 (Asset inventory + Gap matrix)_
_Gap matrix written: 2026-04-30_
